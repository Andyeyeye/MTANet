...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=2, include_top=True):
        self.inplanes = 64
        super(SENet, self).__init__()
        self.include_top = include_top
        
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)       # output wh: 112 * 112
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=0, ceil_mode=True)     # output wh: 56 * 56

        self.layer1 = self._make_layer(block, 64, layers[0])
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)     # output wh: 28 * 28
        ...
        # fine-tuned layers
        self.fc1 = nn.Linear(512 * block.expansion, 1500)    # fc
        self.do1 = nn.Dropout(0.5)
        self.fc2 = nn.Linear(1500, 256)
        self.do2 = nn.Dropout(0.5)
        self.fc3 = nn.Linear(256, num_classes)        # 17: VA:[0:2], AUs:[2:10], Expr:[10:-1]
    ...
    def forward(self, x):
        ...
        if not self.include_top:        # for extract feature 2048-d
            return x
